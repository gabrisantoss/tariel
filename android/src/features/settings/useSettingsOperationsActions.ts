...
export function useSettingsOperationsActions<TCacheLeitura extends CacheLike>({
  ...
  async function handleExportarDiagnosticoApp() {
    ...
    const payload = [
      ...
      `Fila offline: ${offlineQueueTotals.total_items} ${offlineQueueTotals.total_items === 1 ? "item local" : "itens locais"}`,
      ...
      `Fila de suporte: ${filaSuporteLocal.length} ${filaSuporteLocal.length === 1 ? "relato local" : "relatos locais"}`,
      `Fila de suporte com anexo: ${filaSuporteLocal.filter((item) => Boolean(item.attachmentUri)).length} ${filaSuporteLocal.filter((item) => Boolean(item.attachmentUri)).length === 1 ? "relato" : "relatos"}`,
