(function () {
  ...
  const PORTAL_BOOT_CONTRACT = Object.freeze({
    ...
    scriptSelector:
      'script[data-portal-contract="cliente"][data-portal-module]',
    ...
    requiredModules: Object.freeze([
      ...
      Object.freeze({
        key: "TarielClientePortalServicos",
        alias: "servicos",
        requiredForBoot: true,
        temporaryCompat: false,
        dependsOnScriptOrder: true,
      ...
      Object.freeze({
        key: "TarielClientePortalRecorrencia",
        alias: "recorrencia",
        requiredForBoot: true,
        temporaryCompat: false,
        dependsOnScriptOrder: true,
      ...
      Object.freeze({
        key: "TarielClientePortalDocumentos",
        alias: "documentos",
        requiredForBoot: true,
        temporaryCompat: false,
        dependsOnScriptOrder: true,
  ...
  const INITIAL_TAB = [
    ...
    "chat",
    "mesa",
  ].includes(bodyDataset.clienteTabInicial)
    ? bodyDataset.clienteTabInicial
    : "admin";
  const INITIAL_ADMIN_SECTION = [
    "overview",
    "capacity",
    "team",
    "support",
    ...
    : "overview";
  const INITIAL_SERVICOS_SECTION = ["overview"].includes(
    bodyDataset.clienteServicosSectionInicial,
  )
    ? bodyDataset.clienteServicosSectionInicial
    : "overview";
  const INITIAL_RECORRENCIA_SECTION = ["overview"].includes(
    bodyDataset.clienteRecorrenciaSectionInicial,
  )
    ? bodyDataset.clienteRecorrenciaSectionInicial
    : "overview";
  const INITIAL_ATIVOS_SECTION = ["overview"].includes(
    bodyDataset.clienteAtivosSectionInicial,
  )
    ? bodyDataset.clienteAtivosSectionInicial
    : "overview";
  const INITIAL_CHAT_SECTION = ["overview", "new", "queue", "case"].includes(
    bodyDataset.clienteChatSectionInicial,
  )
    ? bodyDataset.clienteChatSectionInicial
    : "overview";
  const INITIAL_MESA_SECTION = [
    "overview",
    "queue",
    "pending",
    "reply",
  ].includes(bodyDataset.clienteMesaSectionInicial)
    ? bodyDataset.clienteMesaSectionInicial
    : "overview";
  const INITIAL_DOCUMENTOS_SECTION = ["overview"].includes(
    bodyDataset.clienteDocumentosSectionInicial,
  )
    ? bodyDataset.clienteDocumentosSectionInicial
    : "overview";
  const ROUTE_MAP = Object.freeze({
    ...
    documentos: bodyDataset.clienteRouteDocumentos || "/cliente/documentos",
  ...
  const state = {
    ...
    ui: {
      ...
      servicosSection: INITIAL_SERVICOS_SECTION,
      recorrenciaSection: INITIAL_RECORRENCIA_SECTION,
      ativosSection: INITIAL_ATIVOS_SECTION,
      documentosSection: INITIAL_DOCUMENTOS_SECTION,
      chatSection: INITIAL_CHAT_SECTION,
      mesaSection: INITIAL_MESA_SECTION,
      sections: {
        admin: INITIAL_ADMIN_SECTION,
        servicos: INITIAL_SERVICOS_SECTION,
        recorrencia: INITIAL_RECORRENCIA_SECTION,
        ativos: INITIAL_ATIVOS_SECTION,
        documentos: INITIAL_DOCUMENTOS_SECTION,
  ...
  function failBoot(reason, options = {}) {
    ...
    const error =
      reason instanceof Error
        ? reason
        : new Error(String(reason || "Falha ao iniciar o portal cliente."));
  ...
  function validateTemplateOrder() {
    const observed = [
      ...document.querySelectorAll(PORTAL_BOOT_CONTRACT.scriptSelector),
    ]
    ...
    const valid =
      ...
      PORTAL_BOOT_CONTRACT.scriptOrder.every(
        (item, index) => observed[index] === item,
      );
  ...
  function resolveBridgeFactories() {
    ...
    const missing = PORTAL_BRIDGE_SPECS.filter(
      (spec) =>
        spec.requiredForBoot &&
        typeof bridgeFactories[spec.alias] !== "function",
    );
  ...
  function createRequiredModule(alias, factory, config) {
    ...
    if (!spec) {
      failBoot(
        `Cliente portal boot contract violado: especificacao ausente para o modulo ${alias}.`,
        {
    ...
    if (typeof factory !== "function") {
      failBoot(
        `Cliente portal boot contract violado: ${spec.key} (${alias}) nao esta disponivel como factory.`,
        {
    ...
    if (!moduleApi || typeof moduleApi !== "object") {
      failBoot(
        `Cliente portal boot contract violado: ${spec.key} (${alias}) nao retornou uma API valida.`,
        {
    ...
    const missingExports = spec.exportShape.filter(
      (nome) => typeof moduleApi[nome] !== "function",
    );
  ...
  const createServicos = bridgeFactories.servicos;
  const createRecorrencia = bridgeFactories.recorrencia;
  const createAtivos = bridgeFactories.ativos;
  const createDocumentos = bridgeFactories.documentos;
  ...
  const prioritiesModule = createRequiredModule(
    "priorities",
    createPriorities,
    {
      helpers: {
  ...
  const sharedHelpersModule = createRequiredModule(
    "sharedHelpers",
    createSharedHelpers,
    {
      documentRef: document,
  ...
  const servicosModule = createRequiredModule("servicos", createServicos, {
    documentRef: document,
    getById: $,
    helpers: {
      escapeAttr,
  ...
  const recorrenciaModule = createRequiredModule(
    "recorrencia",
    createRecorrencia,
    {
      documentRef: document,
  ...
  const ativosModule = createRequiredModule("ativos", createAtivos, {
    documentRef: document,
    getById: $,
    helpers: {
      escapeAttr,
  ...
  const documentosModule = createRequiredModule(
    "documentos",
    createDocumentos,
    {
      documentRef: document,
  ...
  const { bindCommercialActions, bindFiltros, bindTabs } = createRequiredModule(
    "bindings",
    createBindings,
    {
      actions: {
    ...
    return perfAsync(
      ...
      async () => {
        try {
          ...
          window.addEventListener("popstate", () => {
            ...
            if (tab) {
              ...
              bootstrapPortal({
                ...
                .then(() => {
                  if (tab === "admin") {
                    abrirSecaoAdmin(secao, {
                      ensureRendered: true,
                      syncUrl: false,
                    });
                  } else if (tab === "servicos") {
                    abrirSecaoServicos(secao, { syncUrl: false });
                  } else if (tab === "recorrencia") {
                    abrirSecaoRecorrencia(secao, { syncUrl: false });
                  } else if (tab === "ativos") {
                    abrirSecaoAtivos(secao, { syncUrl: false });
                  } else if (tab === "documentos") {
                    abrirSecaoDocumentos(secao, { syncUrl: false });
                  } else if (tab === "chat") {
                ...
                .catch((erro) => {
                  feedback(
                    erro.message ||
                      "Falha ao sincronizar a superfície do portal.",
                    true,
                  );
                });
            }
          ...
          feedback(
            erro.message || "Falha ao carregar o portal admin-cliente.",
            true,
          );
        }
      },
