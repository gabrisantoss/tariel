(function () {
    ...
    window.TarielClientePortalServicosSurface = function createTarielClientePortalServicosSurface(config = {}) {
        ...
        const obterHintVazio = () => state.bootstrap?.guided_onboarding?.surface_empty_hints?.servicos || null;
        ...
        function obterBotoesSecaoServicos() {
            return Array.from(documentRef.querySelectorAll("[data-servicos-section-tab]"));
        }

        function abrirSecaoServicos(secao, { focusTab = false, syncUrl = true } = {}) {
            const secaoAtiva = normalizarSecaoServicos(secao || state.ui?.servicosSection);
            state.ui.servicosSection = secaoAtiva;
            state.ui.sections = state.ui.sections || {};
            state.ui.sections.servicos = secaoAtiva;

            const tabAtiva = obterBotoesSecaoServicos().find((button) => button.dataset.servicosSectionTab === secaoAtiva) || null;
            ...
            documentRef.querySelectorAll("[data-servicos-panel]").forEach((panel) => {
        ...
        function renderServicosResumo() {
            ...
            const totalServices = Number(summary.total_services || items.length || 0);
            ...
            $("servicos-section-count-overview").textContent = `${formatarInteiro(totalServices)} serviços do tenant`;
            ...
            const prioridade = items.find((item) => item.status === "critical")
            ...
            alerta.innerHTML = `
                ...
                <p>${escapeHtml(
                    prioridade.next_due_at
                        ...
                        : prioridade.status_label || "Acompanhe a carteira de serviços da empresa."
                )}</p>
            `;
        }
        ...
        function renderServicosLista() {
            ...
            if (!container) return;
            const items = Array.isArray(state.bootstrap?.servicos?.items) ? state.bootstrap.servicos.items : [];
            if (!items.length) {
                const hint = obterHintVazio();
                container.innerHTML = `
            ...
            container.innerHTML = items.map((item) => `
                <article class="service-card" data-service-key="${escapeAttr(item.service_key || "")}">
                    ...
                    <div class="service-card__meta">
                        <span><strong>Unidades cobertas:</strong> ${escapeHtml(String(item.units_covered || 0))}</span>
                        <span><strong>Ativos cobertos:</strong> ${escapeHtml(String(item.assets_covered || 0))}</span>
                        <span><strong>Pendências:</strong> ${escapeHtml(String(item.open_findings || 0))}</span>
                        <span><strong>Documentos:</strong> ${escapeHtml(String(item.documents_ready || 0))}/${escapeHtml(String(item.documents_total || 0))}</span>
                        <span><strong>Próximas entregas:</strong> ${escapeHtml(item.next_due_at || "Não informada")}</span>
                    </div>
                </article>
            `).join("");
        }
