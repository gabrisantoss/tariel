(function () {
  ...
  window.TarielClientePortalAtivosSurface =
    function createTarielClientePortalAtivosSurface(config = {}) {
      ...
      const obterHintVazio = () =>
        state.bootstrap?.guided_onboarding?.surface_empty_hints?.ativos || null;
      ...
      function obterBotoesSecaoAtivos() {
        return Array.from(
          documentRef.querySelectorAll("[data-ativos-section-tab]"),
        );
      }

      function abrirSecaoAtivos(
        secao,
        { focusTab = false, syncUrl = true } = {},
      ) {
        const secaoAtiva = normalizarSecaoAtivos(
          secao || state.ui?.ativosSection,
        );
        ...
        const tabAtiva =
          obterBotoesSecaoAtivos().find(
            (button) => button.dataset.ativosSectionTab === secaoAtiva,
          ) || null;
      ...
      function renderAtivosResumo() {
        ...
        $("ativos-criticos-total").textContent =
          formatarInteiro(criticalAssets);
        $("ativos-atencao-total").textContent =
          formatarInteiro(attentionAssets);
        $("ativos-saudaveis-total").textContent =
          formatarInteiro(healthyAssets);
        $("ativos-section-count-overview").textContent =
          `${formatarInteiro(totalAssets)} ativos no tenant`;
        ...
        const prioridade =
          items.find((item) => item.health_status === "critical") ||
        ...
        alerta.innerHTML = `
                ...
                <p>${escapeHtml(
                  prioridade.next_due_at
                    ...
                    : prioridade.status_visual_label ||
                        "Acompanhe a carteira de ativos da empresa.",
                )}</p>
            `;
      }
      ...
      function renderAtivosLista() {
        ...
        const items = Array.isArray(state.bootstrap?.ativos?.items)
          ? state.bootstrap.ativos.items
          : [];
        if (!items.length) {
          const hint = obterHintVazio();
          container.innerHTML = `
        ...
        container.innerHTML = items
          .map((item) => {
            ...
            const serviceChips = serviceLabels.length
              ? `
                        <section class="asset-card__section">
                            <div class="asset-card__section-title">Serviços relacionados</div>
                            <div class="asset-card__signals">
                                ${serviceLabels.map((label) => `<span class="asset-card__tag">${escapeHtml(label)}</span>`).join("")}
                            </div>
                        </section>
                    `
              : "";
