        ...
        for empresa_destino_id, email, nome, nivel, senha in usuarios_seed:
            ...
            if usuario:
                ...
                if nivel == int(banco_dados.NivelAcesso.DIRETORIA):
                    ...
                    usuario.mfa_required = False
                    usuario.mfa_secret_b32 = None
                    usuario.mfa_enrolled_at = None
            ...
            if nivel == int(banco_dados.NivelAcesso.DIRETORIA):
                ...
                dados_usuario["mfa_required"] = False
                dados_usuario["mfa_secret_b32"] = None
                dados_usuario["mfa_enrolled_at"] = None
...
def _bootstrap_catalogo_canonico_producao() -> None:
    ...
    from app.domains.admin.catalog_sync_services import sincronizar_catalogo_canonico

    with banco_dados.SessaoLocal() as banco:
        resultado = sincronizar_catalogo_canonico(
            banco,
            status_catalogo="publicado",
        )
        if not resultado["candidate_family_keys"]:
            banco.rollback()
            banco_dados.logger.warning(
                "Bootstrap canônico do catálogo ignorado: nenhum family schema canônico foi encontrado."
            )
            return
        if not resultado["missing_family_keys"]:
            banco.rollback()
            banco_dados.logger.info(
                "Bootstrap canônico do catálogo já sincronizado. %s famílias presentes.",
                resultado["existing_count"],
            )
            return
