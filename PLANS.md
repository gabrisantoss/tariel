...
- `próximo slice sugerido`: continuar drenando `innerHTML` restante em `portal_admin_surface.js` por auditoria ou tabela/resumo de usuários, ou alternar para `mesa/service.py`/`chat_index_page.js`; em paralelo, definir o storage externo/LFS definitivo dos assets migrados para executar restore drill real fora do workspace local
