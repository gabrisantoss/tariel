...
- `próximo slice sugerido`: atacar o último bloco dinâmico relevante de `innerHTML` em `renderUsuarios` no `portal_admin_surface.js`, ou alternar para `mesa/service.py`/`chat_index_page.js`; em paralelo, definir o storage externo/LFS definitivo dos assets migrados para executar restore drill real fora do workspace local
