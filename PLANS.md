...
- `checkpoint UX-E2 2026-04-27`: cards de revisão trocam linguagem interna (`red flag`, `Diff`, `Override`, `Ações canônicas`) por `alerta crítico`, `Histórico de alterações`, `Ajuste humano registrado` e `Ações disponíveis`; `buildThreadContextState` usa `Pendências do caso`, `Status do caso`, `Mesa Avaliadora` e `Família exige Mesa`
...
- `checkpoint UX-E4 2026-04-27`: revisão mobile passa a expor bloco `Revisão` separado de `Pendências do caso`; `Família exige Mesa`, `Mesa Avaliadora` e `Revisão interna governada` ganham descrições próprias, pendências aparecem antes das ações e o trilho de decisão destaca uma ação principal por estado com alternativas em `Ações secundárias`
...
- `checkpoint 2026-04-27`: PR I generaliza guardrails de familias de alto risco em `high_risk_family_guardrails.py`; NR35 Linha de Vida preserva `nr35_mesa_required_unavailable`, enquanto NR35 Ponto de Ancoragem, NR13 Caldeira, NR13 Vaso de Pressao, NR20 Prontuario e NR10 Prontuario usam `high_risk_mesa_required_unavailable`; familia simples sem Mesa continua em self-review, e familia alto risco com policy `mobile_autonomous` e pacote com Mesa e forcada para `mesa_required`
