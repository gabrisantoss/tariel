...
- `status`: em andamento localmente em `2026-05-10`; objetivo e corrigir o desvio de produto que transformou a Mesa Avaliadora em caminho obrigatorio, recolocando mobile e chat web como fluxo principal do Tariel
- `checkpoint 2026-05-10`: documentos canonicos atualizados para tratar Mesa como camada opcional de revisao humana; ela so deve ser exigida quando pacote/politica do tenant declarar isso explicitamente
- `checkpoint 2026-05-10`: guardrails de familias de alto risco passam a emitir recomendacao de Mesa em vez de bloqueio universal; tenants sem Mesa e com aprovacao interna liberada podem finalizar no chat/mobile com aviso auditavel quando o gate tecnico do caso estiver completo
- `checkpoint 2026-05-10`: criado `docs/product-canonical-vision/05_master_product_map_2026-05-10.md` como mapa mestre para recentralizar o Tariel em app/chat de inspecao com IA, separar responsabilidades por perfil e definir o proximo corte como auditoria do Chat Web contra o Mobile
- `checkpoint 2026-05-10`: criada `docs/product-canonical-vision/06_mobile_vs_web_chat_audit_2026-05-10.md`, concluindo que o app Android deve guiar o modelo mental do Chat Web com navegacao primaria `Chat`, `Revisao` e `Finalizar`, deixando `Historico`, `Anexos`, `Correcoes`, preview, readiness e emissao como contexto secundario
- `checkpoint 2026-05-10`: iniciado `WEB-CHAT-MOBILE-MIRROR-01`; toolbar do workspace web agora separa fluxo principal (`Chat`, `Revisao`, `Finalizar`) de apoios secundarios (`Historico`, `Anexos`, `Correcoes`), e a aba `mesa` volta a abrir a superficie de revisao em vez de cair em correcoes
- `checkpoint 2026-05-10`: iniciado `WEB-CHAT-MOBILE-MIRROR-02`; composer do Chat Web reduz helpers persistentes, oculta a previa duplicada do bloco de digitacao, remove chips longos do seletor de NR e deixa sugestoes automaticas restritas a avisos governados
- `checkpoint 2026-05-10`: concluido `WEB-CHAT-MOBILE-MIRROR-03`; `Finalizar` deixa de ser apenas botao solto da toolbar e vira view propria `inspection_finalization`, com previa, acao final e atalhos para pendencias, anexos, chat e revisao
- `checkpoint 2026-05-10`: iniciado `WEB-CHAT-MOBILE-MIRROR-04` por feedback visual em tela real; conversa curta com resposta longa nao deve usar alinhamento `flex-end`, porque isso pode cortar o topo da mensagem do inspetor/IA; o chat passa a usar topo fixo com respiro e folga inferior suficiente para o composer nao cobrir mensagens
- `checkpoint 2026-05-10`: chat livre passa a seguir modelo ChatGPT/Gemini: rail operacional e header tecnico saem do caminho, resposta da IA fica leve/sem cardao, bolha do usuario fica simples a direita e o composer permanece como foco principal
- `checkpoint 2026-05-10`: ajuste de feedback: o rail operacional nao deve aparecer por padrao no chat livre, mas deve continuar abrindo pelo botao lateral; removida a trava que impedia o toggle manual nesse modo
- `checkpoint 2026-05-10`: refinado comportamento Gemini-like do chat livre: respostas da IA deixam de renderizar como card de documento, metadados tecnicos/confianca ficam fora da conversa livre, e o historico passa a fluir pela area principal sem scrollbar interna competindo com o composer
- `checkpoint 2026-05-10`: corrigido feedback visual imediato: pais do chat livre deixam de travar `height: 100%`/`overflow: hidden`, permitindo rolagem vertical da conversa quando a resposta fica maior que a tela
- `checkpoint 2026-05-10`: reforcado fallback do scroll Gemini-like tambem quando o estado `freeChatConversationActive` ainda nao sincronizou; conversa sem laudo ativo libera scroll no painel principal e reserva folga inferior para o composer nao cobrir a ultima mensagem
- `checkpoint 2026-05-10`: segunda passada Gemini-like: landing passa para saudacao curta `Ola, <nome>` + `Por onde comecamos?`, composer usa placeholder `Peca ao Tariel`, conversa livre ganha titulo discreto, botao `Laudo` sai do fluxo principal e a lateral esquerda vira rail compacto expansivel por hover/foco no desktop
- `checkpoint 2026-05-11`: removido visualmente o toggle antigo da lateral esquerda (`btn-sidebar-edge-toggle`) no landing/chat livre Gemini-like; a lateral compacta continua acessivel por hover/foco e o controle redundante deixa de poluir o topo
- `checkpoint 2026-05-11`: rail direito passa a ter modo hover no landing/chat livre quando estiver disponivel; o painel fica off-canvas na borda direita e desliza por hover/foco, permitindo remover tambem o botao fixo de esconder/mostrar ferramentas
- `checkpoint 2026-05-11`: teste visual sem bolhas nas mensagens do chat livre; removidos fundo, borda, raio, sombra e background-image dos cards de usuario e IA para manter a separacao sem bloco atras da conversa
- `checkpoint 2026-05-11`: o mesmo tratamento sem bloco foi aplicado a timeline do Historico, removendo card visual de `workspace-history-card` sem ocultar icone, texto, anexos ou acoes
- `checkpoint 2026-05-11`: removida piscada da home apos login do inspetor; `/app/` sem laudo continua limpando contexto ativo, mas agora renderiza o workspace/chat livre direto no SSR, deixando a home apenas para `/app/?home=1`
- `checkpoint 2026-05-11`: hotfix de travamento no Chrome apos refinamentos Gemini-like; seletores `:has()` amplos em `body`/superficie de chat foram trocados por `data-*` sincronizados no runtime para evitar reavaliacao pesada durante mutacoes da conversa
- `checkpoint 2026-05-11`: bolhas/cards de mensagem removidos de forma global no workspace do inspetor, cobrindo conversa, chat livre, mensagens de IA/usuario/sistema/mesa e historico, sem depender de estado sincronizado do laudo
- `checkpoint 2026-05-11`: corrigido travamento ao clicar em `Chat livre`; a limpeza interna do novo chat nao reemite `tariel:estado-relatorio`, evitando loop de restauracao/limpeza, e o historico lateral tambem perde card/bolha visual
- `checkpoint 2026-05-11`: reset sem bolhas movido tambem para a camada base `reboot`, porque cards antigos com `!important` em CSS Layers posteriores venciam o refinamento visual; mensagens e historico agora removem borda/fundo/sombra direto na camada que prevalece
- `checkpoint 2026-05-11`: refinamento ChatGPT-like aplicado corretamente: IA fica como texto solto, mensagem do usuario volta como pilula compacta a direita e o composer perde a borda interna do textarea, mantendo uma capsula principal mais limpa
- `checkpoint 2026-05-11`: pilula do usuario passa a ser automatica por conteudo (`fit-content` + limite maximo), crescendo para mensagens medias e quebrando linhas com seguranca em textos longos sem virar card fixo
- `proximo passo recomendado`: seguir no `WEB-CHAT-MOBILE-MIRROR-04`, fazendo dieta de header/readiness e transformando o rail lateral em apoio opcional, sem competir com a conversa
- `validacao WEB-CHAT-MOBILE-MIRROR-04 no-home-flash 2026-05-11`: `cd web && .\.venv\Scripts\python.exe -m pytest tests/test_inspector_active_report_authority.py -q` (`4 passed`); `cd web && .\.venv\Scripts\python.exe -m py_compile app/domains/chat/session_helpers.py tests/test_inspector_active_report_authority.py`; `git diff --check -- web/app/domains/chat/session_helpers.py web/tests/test_inspector_active_report_authority.py PLANS.md` (sem erro; aviso CRLF conhecido em `session_helpers.py`)
- `validacao WEB-CHAT-MOBILE-MIRROR-04 invisible-bubbles 2026-05-11`: `cd web && .\.venv\Scripts\python.exe -m pytest tests/test_smoke.py -q -k templates_chat_mantem_controles_essenciais_de_ui` (`1 passed, 51 deselected`); `git diff --check -- web/static/css/inspetor/visual_refinements.css web/templates/inspetor/base.html web/tests/test_smoke.py PLANS.md`; `http://127.0.0.1:8000/health` (`200`)
- `validacao WEB-CHAT-MOBILE-MIRROR-04 history-invisible-bubbles 2026-05-11`: `cd web && .\.venv\Scripts\python.exe -m pytest tests/test_smoke.py -q -k templates_chat_mantem_controles_essenciais_de_ui` (`1 passed, 51 deselected`); `git diff --check -- web/static/css/inspetor/visual_refinements.css web/templates/inspetor/base.html web/tests/test_smoke.py PLANS.md`; `http://127.0.0.1:8000/health` (`200`)
- `validacao WEB-CHAT-MOBILE-MIRROR-04 chat-loop-guard 2026-05-11`: `node --check web/static/js/shared/api.js`; `node --check web/static/js/inspetor/workspace_context_flow.js`; `node --check web/static/js/inspetor/system_events.js`; `cd web && .\.venv\Scripts\python.exe -m pytest tests/test_smoke.py -q -k templates_chat_mantem_controles_essenciais_de_ui` (`1 passed, 51 deselected`); `git diff --check -- web/static/js/shared/api.js web/static/js/inspetor/workspace_context_flow.js web/static/js/inspetor/system_events.js web/templates/index.html web/templates/inspetor/base.html web/static/css/inspetor/visual_refinements.css web/tests/test_smoke.py PLANS.md` (sem erro; aviso CRLF em `api.js`); `http://127.0.0.1:8000/health` (`200`)
- `validacao WEB-CHAT-MOBILE-MIRROR-04 no-message-wrappers 2026-05-11`: `cd web && .\.venv\Scripts\python.exe -m pytest tests\test_smoke.py -q -k templates_chat_mantem_controles_essenciais_de_ui` (`1 passed, 51 deselected`); CSS servido em `http://127.0.0.1:8000/static/css/inspetor/reboot.css?v=dev&ui_rev=20260511_chat_no_message_wrappers` contem o reset de bolhas; `git diff --check -- web/static/css/inspetor/reboot.css web/static/css/inspetor/visual_refinements.css web/templates/inspetor/base.html web/tests/test_smoke.py PLANS.md`
- `validacao WEB-CHAT-MOBILE-MIRROR-04 chatgpt-like-thread 2026-05-11`: `cd web && .\.venv\Scripts\python.exe -m pytest tests\test_smoke.py -q -k templates_chat_mantem_controles_essenciais_de_ui` (`1 passed, 51 deselected`); CSS servido em `http://127.0.0.1:8000/static/css/inspetor/reboot.css?v=dev&ui_rev=20260511_chatgpt_like_thread` contem conversa/composer ChatGPT-like; `git diff --check -- web/static/css/inspetor/reboot.css web/static/css/inspetor/visual_refinements.css web/templates/inspetor/base.html web/tests/test_smoke.py PLANS.md`
- `validacao WEB-CHAT-MOBILE-MIRROR-04 auto-user-pill 2026-05-11`: `cd web && .\.venv\Scripts\python.exe -m pytest tests\test_smoke.py -q -k templates_chat_mantem_controles_essenciais_de_ui` (`1 passed, 51 deselected`); CSS servido em `http://127.0.0.1:8000/static/css/inspetor/reboot.css?v=dev&ui_rev=20260511_auto_user_pill` contem `fit-content`, `inline-flex` e `pre-wrap`; `git diff --check -- web/static/css/inspetor/reboot.css web/templates/inspetor/base.html web/tests/test_smoke.py PLANS.md`
- `validacao WEB-CHAT-MOBILE-MIRROR-04 perf-no-has 2026-05-11`: `rg -n "body\\.pagina-chat-dashboard-v2:has|chat-thread-surface:has|:has\\(\\.chat-dashboard-grid|:has\\(#workspace-conversation-empty|:has\\(#painel-chat" web/static/css/inspetor/visual_refinements.css web/static/css/inspetor/workspace_states.css web/static/css/shared/app_shell.css` (sem ocorrencias); `node --check web/static/js/inspetor/workspace_derivatives.js`; `node --check web/static/js/shared/chat-render.js`; `cd web && .\.venv\Scripts\python.exe -m pytest tests/test_smoke.py -q -k templates_chat_mantem_controles_essenciais_de_ui` (`1 passed, 51 deselected`); `git diff --check -- web/static/css/inspetor/visual_refinements.css web/static/css/inspetor/workspace_states.css web/static/css/shared/app_shell.css web/static/js/inspetor/workspace_derivatives.js web/static/js/shared/chat-render.js web/templates/inspetor/base.html web/tests/test_smoke.py PLANS.md` (sem erro; avisos CRLF em `app_shell.css` e `chat-render.js`); `http://127.0.0.1:8000/health` (`200`)
- `validacao WEB-CHAT-MOBILE-MIRROR-04 rails 2026-05-11`: `node --check web/static/js/inspetor/workspace_rail.js`; `node --check web/static/js/chat/chat_index_page.js`; `cd web && .\.venv\Scripts\python.exe -m pytest tests/test_smoke.py -q -k "templates_chat_mantem_controles_essenciais_de_ui or inspector_workspace_shell"` (`1 passed, 51 deselected`); `cd web && .\.venv\Scripts\python.exe -m pytest tests/test_smoke.py -q` (`52 passed`); `git diff --check -- web/static/css/inspetor/visual_refinements.css web/static/js/inspetor/workspace_rail.js web/static/js/chat/chat_index_page.js web/templates/inspetor/base.html web/tests/test_smoke.py PLANS.md`; `http://127.0.0.1:8000/health` (`200`)
- `validacao WEB-CHAT-MOBILE-MIRROR-04 scroll 2026-05-10`: `cd web && .\.venv\Scripts\python.exe -m pytest tests/test_smoke.py -q -k templates_chat_mantem_controles_essenciais_de_ui` (`1 passed, 51 deselected`); `cd web && .\.venv\Scripts\python.exe -m pytest tests/test_smoke.py -q` (`52 passed`); `git diff --check -- web/static/css/inspetor/visual_refinements.css web/templates/inspetor/base.html web/tests/test_smoke.py PLANS.md`; `http://127.0.0.1:8000/health` (`200`)
- `validacao WEB-CHAT-MOBILE-MIRROR-03 2026-05-10`: `node --check` nos JS afetados; `cd web && .\.venv\Scripts\python.exe -m pytest tests/test_smoke.py -q -k templates_chat_mantem_controles_essenciais_de_ui` (`1 passed, 51 deselected`); `cd web && .\.venv\Scripts\python.exe -m pytest tests/test_smoke.py -q` (`52 passed`); `git diff --check -- ...` sem erros bloqueantes, apenas aviso CRLF ja conhecido em `web/static/js/chat/chat_painel_core.js`
- `validacao 2026-05-10`: `cd web && .\.venv\Scripts\python.exe -m py_compile app/domains/chat/high_risk_family_guardrails.py app/domains/chat/laudo_decision_services.py app/domains/cliente/dashboard_documents_support.py app/domains/cliente/dashboard_overview_support.py`; `node --check web/static/js/revisor/revisor_painel_core.js`; `cd web && .\.venv\Scripts\python.exe -m pytest tests/test_high_risk_family_guardrails_pr_i.py tests/test_mobile_only_client_no_mesa_pr_f.py tests/test_company_no_mesa_multi_user_pr_g.py tests/test_tenant_entitlements_critical.py tests/test_mobile_chat_first_capability_aliases.py -q` (`37 passed`); `cd web && .\.venv\Scripts\python.exe -m pytest tests/test_smoke.py -q` (`52 passed`)
...
- `checkpoint 2026-05-08`: `scripts/run_mobile_maestro_smoke.ps1` agora procura Maestro tambem em `%USERPROFILE%\.maestro\bin` e recarrega variaveis de ambiente de usuario
- `checkpoint 2026-05-08`: runners de aceitacao/gates agora resolvem Python, npm/Maestro, Git Bash e Android SDK no Windows sem depender de `python3` ou executaveis POSIX diretos
- `checkpoint 2026-05-08`: smoke mobile real passou no AVD `tariel_api36` com APK novo, `feed_covered=True`, `thread_covered=True`, `operator_run_outcome=completed_successfully` e `human_ack_recent_events_after=2`
- `validacao executada`: `.\make.ps1 doctor`; `.\make.ps1 web-ci`; `.\make.ps1 mobile-ci`; `.\make.ps1 mesa-smoke`; `.\make.ps1 verify`; `.\make.ps1 mesa-acceptance`; `.\make.ps1 document-acceptance`; `.\make.ps1 observability-acceptance`; `.\make.ps1 smoke-mobile`; `.\make.ps1 release-verify`
...
### `PKT-UX-PRODUCT-LANGUAGE-03` - Portal Cliente Documentos shell de leitura

- `status`: concluido localmente em `2026-04-27`; UX-C aprofunda o piloto da aba `Documentos` com hierarquia visual mais clara e menos ruido de chips, sem alterar regra de negocio, emissao oficial, permissao, release gate ou mobile smoke
- `checkpoint 2026-04-27`: `web/templates/cliente/documentos/_content.html` passa a explicitar leitura por blocos (`Resumo`, `Documento oficial`, `Historico`, `Auditoria` e `Recursos do pacote`) e reforca que PDF operacional continua separado da emissao oficial
- `checkpoint 2026-04-27`: `web/static/js/cliente/portal_documentos_surface.js` reorganiza cada card documental em secoes estaveis (`Resumo`, `Emissao oficial/PDF operacional`, `Historico`, `Auditoria`, `Sinais` e `Recursos`), substitui parte dos chips por linhas de leitura e mantem `Baixar pacote oficial` como CTA principal quando houver emissao ativa
- `checkpoint 2026-04-27`: `web/static/css/cliente/portal_documentos_surface.css` adiciona estilos de blocos (`document-card__row`, `document-card__badge`, `document-summary-card__blocks`) para melhorar escaneabilidade sem criar nova arquitetura frontend
- `validacao executada`: `node --check web/static/js/cliente/portal_documentos_surface.js`; `cd web && PYTHONPATH=. python -m pytest tests/test_smoke.py -q -k ux_product_language_padroniza_documentos_do_portal_cliente` (`1 passed`); `git diff --check`

### `PKT-UX-PRODUCT-LANGUAGE-04` - Portal Cliente Chat/Mesa status e shell

- `status`: concluido localmente em `2026-04-27`; UX-D padroniza status e reduz ruído visual em `Chat` e `Mesa` do Portal Cliente sem alterar regras de negocio, endpoints, emissao oficial, permissao, release gate ou mobile smoke
- `checkpoint 2026-04-27`: `portal_chat_surface.js` e `portal_mesa_surface.js` alinham lifecycle/status para linguagem canônica (`Em coleta`, `Na Mesa`, `Pendente`, `Aprovado`, `Emitido`) e removem fallback `Fluxo legado`
- `checkpoint 2026-04-27`: sinais canônicos da Mesa deixam de usar rótulo cru `Owner`, passando para `Responsavel`; labels de decisão/emissão ficam mais explícitas (`Decisao da Mesa disponivel`, `Pronto para emissao oficial`, `Documento emitido`)
- `checkpoint 2026-04-27`: `portal_chat_surface.css` e `portal_mesa_surface.css` deixam `surface-nav` dessas superfícies em `position: static`, reduzindo empilhamento de barras sticky durante scroll
- `checkpoint 2026-04-27`: `portal_servicos_surface.js` e `portal_ativos_surface.js` reduzem ruido de chips em cards operacionais (linhas de leitura para servicos e secao titulada para servicos relacionados em ativos)
- `checkpoint 2026-04-27`: `portal_servicos_surface.css`, `portal_ativos_surface.css` e `portal_recorrencia_surface.css` deixam subnav dessas superfícies em `position: static`, mantendo shell com menos poluicao durante scroll
- `checkpoint 2026-04-27`: `portal_admin_surface.js` troca `Owner predominante` por `Responsavel predominante`; `portal_admin_surface.css` aplica subnav nao-sticky no Painel Admin Cliente
- `checkpoint 2026-04-27`: `inspetor/preparar_emissao.html` padroniza copy de reemissao para separar emissao oficial e PDF operacional (`Mudancas desde a emissao ativa`, `Mudanca no PDF operacional`, `Acao sugerida`)
- `checkpoint 2026-04-27`: `templates/painel_revisor.html` padroniza sinais canônicos da fila da Mesa (`Status`, `Responsavel`, `Decisao da Mesa disponivel`) removendo termos legados (`Fluxo legado`, `Owner`) sem mexer no payload
- `checkpoint 2026-04-27`: `static/css/revisor/painel_revisor.css` desativa sticky nos cabeçalhos de seção da lista (`.lista-scroll .lista-header`) para reduzir empilhamento visual no scroll
- `checkpoint 2026-04-27`: `templates/cliente/_primary_tabs.html` substitui o bloco grande de superfícies por shell compacto com barra sticky + botão `Áreas` e dropdown (`Admin`, `Serviços`, `Recorrência`, `Ativos`, `Documentos`, `Chat`, `Mesa Avaliadora`) preservando links, badges/counts e indisponibilidade por pacote
- `checkpoint 2026-04-27`: `static/js/cliente/portal_bindings.js` passa a controlar o dropdown do shell (`aria-expanded`, abrir/fechar, clique fora, `Escape`, foco inicial e fechamento após seleção) sem alterar permissões ou rotas
- `checkpoint 2026-04-27`: `static/js/cliente/portal_runtime.js` sincroniza resumo da área ativa na barra (`cliente-shell-current-title/meta`) a cada troca de superfície
- `validacao executada`: `node --check web/static/js/cliente/portal_chat_surface.js web/static/js/cliente/portal_mesa_surface.js`; `python -m py_compile web/tests/test_smoke.py`; `cd web && PYTHONPATH=. python -m pytest tests/test_smoke.py -q -k 'ux_d_padroniza_status_chat_mesa_sem_owner_cru or templates_cliente_explicitam_abas_e_formularios_principais'` (`2 passed`); `npx prettier --check web/static/js/cliente/portal_chat_surface.js web/static/js/cliente/portal_mesa_surface.js web/static/css/cliente/portal_chat_surface.css web/static/css/cliente/portal_mesa_surface.css`; `git diff --check`
- `validacao shell compacta 2026-04-27`: `node --check web/static/js/cliente/portal_bindings.js web/static/js/cliente/portal_runtime.js`; `npx prettier --check web/static/js/cliente/portal_bindings.js web/static/js/cliente/portal_runtime.js web/static/css/cliente/portal.css web/static/css/cliente/portal_admin_theme.css`; `python -m py_compile web/tests/test_smoke.py`; `cd web && PYTHONPATH=. python -m pytest tests/test_smoke.py -q -k 'templates_cliente_explicitam_abas_e_formularios_principais or ux_d_padroniza_status_chat_mesa_sem_owner_cru'` (`2 passed`); `git diff --check`
- `validacao complementar 2026-04-27`: `node --check web/static/js/cliente/portal_servicos_surface.js web/static/js/cliente/portal_ativos_surface.js web/static/js/cliente/portal_admin_surface.js`; `python -m py_compile web/tests/test_smoke.py web/tests/test_tenant_entitlements_critical.py web/tests/test_reviewer_panel_boot_hotfix.py`; `cd web && PYTHONPATH=. python -m pytest tests/test_smoke.py -q -k 'ux_d_padroniza_status_chat_mesa_sem_owner_cru or ux_d_reduz_ruido_visual_em_servicos_ativos_recorrencia or ux_d_padroniza_linguagem_preparar_emissao_inspetor or ux_d_padroniza_admin_sem_owner_cru or ux_d_padroniza_painel_revisor_sem_owner_cru or templates_cliente_explicitam_abas_e_formularios_principais'` (`6 passed`); `cd web && PYTHONPATH=. python -m pytest tests/test_tenant_entitlements_critical.py -q -k preparacao_emissao_exibe_contexto_documental_e_fotos_curadas` (`1 passed`); `cd web && PYTHONPATH=. python -m pytest tests/test_reviewer_panel_boot_hotfix.py -q -k ssr_expoe_sinais_canonicos_do_caso_na_fila` (`1 passed`); `npx prettier --check web/static/js/cliente/portal_servicos_surface.js web/static/js/cliente/portal_ativos_surface.js web/static/js/cliente/portal_admin_surface.js web/static/css/cliente/portal_servicos_surface.css web/static/css/cliente/portal_ativos_surface.css web/static/css/cliente/portal_recorrencia_surface.css web/static/css/cliente/portal_admin_surface.css`; `git diff --check`

### `PKT-UX-PRODUCT-LANGUAGE-05` - Mobile polish (home/thread/atividade)

- `status`: em andamento localmente em `2026-04-27`; UX-E aplica linguagem canônica no app mobile sem alterar regra de negócio, endpoints, permissões, emissão oficial, NR35, release gate, Maestro ou `human_ack`
- `checkpoint 2026-04-27`: `ThreadConversationReportPackDraftCard` separa explicitamente `PDF operacional` da trilha de `Emissão oficial`; `ThreadConversationIssuedDocumentCard` passa a destacar `Emissão oficial` com copy de divergência focada em `PDF operacional`
- `checkpoint 2026-04-27`: `threadContextFinalization` e `HistoryDrawerListItem` removem rótulos internos visíveis de `Owner` para `Responsável`; rótulos de revisão foram consolidados para `Revisão interna governada` e `Revisão interna + Mesa`
- `checkpoint 2026-04-27`: `buildInspectorBaseDerivedStateSections` atualiza shell mobile para `Revisão interna governada` quando Mesa separada não é exigida
- `checkpoint UX-E2 2026-04-27`: home operacional reduz repetição do card de caso; `ThreadContextCard` e `ThreadFinalizationCard` passam a separar `Próxima ação`, `Status e pendências` e `Documento e auditoria`, deixando só uma ação principal por estado e ações secundárias em trilho discreto
- `checkpoint UX-E2 2026-04-27`: central de atividade ganha resumo compacto por novas notificações, reemissão recomendada, Mesa Avaliadora e status do caso; títulos/corpos são sanitizados para não exibir `mobile_autonomous`, `mobile_review_allowed`, `primary_pdf_diverged`, `issue_state`, `superseded`, `reviewer_issue` ou `reviewer_decision`
- `checkpoint UX-E2 2026-04-27`: cards de revisão trocam linguagem interna (`red flag`, `Diff`, `Override`, `Ações canônicas`) por `alerta crítico`, `Histórico de alterações`, `Ajuste humano registrado` e `Ações disponíveis`; `buildThreadContextState` usava `Pendências do caso`, `Status do caso`, `Mesa Avaliadora` e um rótulo histórico de exigência de Mesa, supersedido em `2026-05-10` por `Família recomenda Mesa`
- `checkpoint UX-E3 2026-04-27`: frente aberta para reduzir densidade da thread do caso e reorganizar cards de `PDF operacional`, `Emissão oficial`, `Pacote oficial`, `Revisão interna governada`, `Mesa Avaliadora`, `Pendências do caso`, histórico/auditoria e reemissão recomendada, preservando testIDs/acessibilidade do smoke e sem alterar regra de negócio
- `checkpoint UX-E3 2026-04-27`: thread do caso passa a separar visualmente `PDF operacional` e `Emissão oficial`, move detalhes técnicos para texto secundário, usa `Reemissão recomendada` como alerta contextual, reduz metadados do card operacional e sanitiza termos internos (`mobile_autonomous`, `mobile_review_allowed`, `primary_pdf_diverged`, `issue_state`, `superseded`, `reviewer_issue`, `reviewer_decision`, `override`, `diff`, `red flag`)
- `validacao UX-E3 2026-04-27`: `cd android && npm test -- --runInBand src/features/chat/ThreadConversationPane.test.tsx src/features/chat/reportPackHelpers.test.ts src/features/chat/ThreadFinalizationCard.test.tsx src/features/chat/buildThreadContextState.test.ts` (`39 passed`); `cd android && npm test -- --runInBand src/features/settings/exportDataFlow.test.ts` (`2 passed`); `cd android && npm run typecheck`; `cd android && npm run lint`; `cd android && npm run format:check`; `make mobile-ci` (`113 suites`, `432 tests`); `git diff --check`; `smoke-mobile` não executado porque testIDs/navegação/labels usados pelo Maestro foram preservados
- `checkpoint UX-E4 2026-04-27`: revisão mobile passa a expor bloco `Revisão` separado de `Pendências do caso`; o rótulo histórico de exigência de Mesa foi supersedido em `2026-05-10` por `Família recomenda Mesa`, mantendo `Mesa Avaliadora` e `Revisão interna governada` como descrições próprias
- `validacao UX-E4 2026-04-27`: `cd android && npm test -- --runInBand src/features/chat/ThreadConversationPane.test.tsx src/features/activity/activityNotificationHelpers.test.ts src/features/chat/ThreadFinalizationCard.test.tsx src/features/chat/buildThreadContextState.test.ts src/features/common/OperationalModals.test.tsx` (`49 passed`); `cd android && npm run typecheck`; `cd android && npm run lint`; `cd android && npm run format:check`; `make mobile-ci` (`113 suites`, `435 tests`); `git diff --check`; `smoke-mobile` não executado porque testIDs/navegação/labels usados pelo Maestro foram preservados
- `checkpoint UX-E5 2026-04-27`: bloco documental mobile reorganizado para separar `PDF operacional`, `Emissão oficial`, `Download oficial`, `Auditoria`, `Histórico de emissões`, `Documento substituído` e `Reemissão recomendada`; download principal fica restrito à emissão oficial ativa, enquanto documento substituído aparece como histórico sem CTA ativo
- `checkpoint UX-E5 2026-04-27`: hash/pacote/snapshot deixam de aparecer como termos crus no card principal; `package_sha256` vira `Hash do pacote`, `issued` vira `Documento emitido`, `superseded` vira `Documento substituído` e `revoked` vira `Histórico de emissões`
- `checkpoint UX-E5 2026-04-27`: CTA `Abrir Mesa Avaliadora` do report pack foi mantido com testID estável e reposicionado depois de `Pendências do caso`, preservando a leitura de bloqueio antes da ação e melhorando alcance no smoke em dispositivo físico
- `validacao UX-E5 2026-04-27`: `cd android && npm test -- --runInBand src/features/chat/ThreadConversationPane.test.tsx src/features/chat/reportPackHelpers.test.ts src/features/activity/activityNotificationHelpers.test.ts src/features/common/OperationalModals.test.tsx` (`34 passed`); `cd android && npm run typecheck`; `cd android && npm run lint`; `cd android && npm run format:check`; `make mobile-ci` (`113 suites`, `439 tests`); `make smoke-mobile` em USB físico `RQCW20887GV` foi tentado, mas a última rodada ficou inconclusiva antes do login porque o aparelho entrou em lockscreen/AOD (`app_opened_but_target_not_reached`, artefato `artifacts/mobile_pilot_run/20260427_205352`)
- `validacao executada`: `cd android && npm run typecheck`; `cd android && npm run lint`; `cd android && npm run format:check`; `cd android && npm test -- --runInBand src/features/chat/ThreadConversationPane.test.tsx src/features/chat/ThreadFinalizationCard.test.tsx src/features/history/HistoryDrawerPanel.test.tsx src/features/common/buildInspectorBaseDerivedStateSections.test.ts`; `cd android && npm test -- --runInBand src/features/chat/buildThreadContextState.test.ts src/features/activity/activityNotificationHelpers.test.ts src/features/settings/exportDataFlow.test.ts`; `make mobile-ci`; `git diff --check`
- `validacao UX-E2 2026-04-27`: `cd android && npm test -- --runInBand src/features/chat/ThreadContextCard.test.tsx src/features/chat/ThreadFinalizationCard.test.tsx src/features/common/OperationalModals.test.tsx src/features/activity/activityNotificationHelpers.test.ts src/features/chat/buildThreadContextState.test.ts src/features/chat/ThreadConversationPane.test.tsx` (`51 passed`); `cd android && npm run typecheck`; `cd android && npm run lint`; `cd android && npm run format:check`; `make mobile-ci` (`113 suites`, `431 tests`); `git diff --check`
...
- `checkpoint 2026-04-26`: a previa de finalizacao do Chat Inspetor passa a retornar `mobile_chat_first_governance` e `chat_review_tools`; a modal usa os rotulos neutros quando presentes, permitindo `Revisao interna governada`, `Pendencias do caso` e `Revisao pela Mesa Avaliadora` sem remover `primary_action` ou `primary_label` legados
- `checkpoint 2026-04-26`: a rail do Chat Inspetor passa a condicionar templates/editor/assinatura/PDF/pacote por aliases neutros (`structured_review_edit`, `governed_signatory_select`, `official_issue_create`, `official_issue_download`), com `app_shell.js` resolvendo aliases e capabilities legadas para preservar pacotes existentes
- `checkpoint 2026-04-26`: o Mobile passa a tipar e resolver `capability_aliases`, `user_capability_aliases`, `mobile_chat_first_governance` e `user_mobile_chat_first_governance`; `mobileUserAccess` aceita aliases neutros com fallback legado e `buildInspectorConversationDerivedState` expõe `selfReviewAllowed`, `separateMesaRequired`, `officialIssueAllowed`, `officialIssueDownloadAllowed`, `reviewSurfaceLabel`, `pendingCaseLabel` e `officialIssueLabel`
- `checkpoint 2026-04-26`: labels mobile deixam de expor `mobile_autonomous`/`Autonomia mobile`/`Aprovar no mobile` e passam a usar `Revisao interna`/`Aprovar internamente`; quando `separate_mesa_required=true`, o app mantém `Mesa Avaliadora`; PDF operacional continua sendo anexo/relatorio e nao e tratado como emissao oficial
- `checkpoint 2026-04-26`: o Portal Cliente/Admin Cliente passa a publicar `operating_model`, `capability_aliases`, `mobile_chat_first_governance`, `resource_summary` e a matriz visual `Seu pacote e recursos`; a UI diferencia `Mobile`, `Chat Inspetor`, `Revisao interna`, `Mesa Avaliadora`, `Emissao oficial`, `Documentos oficiais` e `Signatario governado`, tratando ausencia de Mesa/emissao como limite de pacote quando aplicavel
- `checkpoint 2026-04-26`: o Mobile passa a receber `download_url` do pacote oficial ativo dentro de `reviewPackage.emissao_oficial.current_issue` e renderiza `Baixar pacote oficial` na thread e na superficie Mesa usando o downloader autenticado existente; nao cria emissao oficial nova pelo app
- `checkpoint 2026-04-26`: downloads de emissao oficial passam a registrar auditoria sanitizada para Portal Cliente, Chat/Mobile pelo endpoint `/app` e Mesa pelo endpoint `/revisao`, preservando tenant, `issue_number`, hash e tipo de artefato sem expor path local de storage
- `checkpoint 2026-04-27`: PR F adiciona teste integrado de cliente individual/mobile-only com pacote `inspector_chat`, `operating_model=mobile_single_operator`, familia simples `padrao`, revisao interna governada sem Mesa, Portal Cliente comunicando Mobile/Chat liberados e Mesa/emissao oficial nao incluidas, Mobile sem `official_issue_summary` quando nao ha `EmissaoOficialLaudo`; o bloqueio NR35 historico foi supersedido em `2026-05-10` por aviso auditavel de Mesa recomendada
- `checkpoint 2026-04-27`: PR G adiciona teste integrado de empresa sem Mesa com multiplos inspetores, pacote `inspector_chat`, `operating_model=standard`, Admin Cliente gerenciando equipe sem virar Admin CEO, inspetor criando/finalizando familia simples sem handoff para Mesa, isolamento entre inspetores da mesma empresa, Portal Cliente sem emissao oficial/download oficial, Mobile sem `official_issue_summary`; o bloqueio NR35 historico foi supersedido em `2026-05-10` por aviso auditavel de Mesa recomendada
- `checkpoint 2026-04-27`: PR H adiciona teste integrado de empresa premium com Mesa, pacote `inspector_chat_mesa`, `operating_model=standard`, familia simples modelada `cbmgo`, handoff para Mesa, aprovacao humana, snapshot aprovado, PDF operacional sem ser oficial antes da emissao, `EmissaoOficialLaudo` ativa com signatario governado, pacote congelado, Portal Cliente consumindo a emissao ativa e Mobile/Chat apontando para download oficial
- `checkpoint 2026-04-27`: PR I generalizou guardrails de familias de alto risco em `high_risk_family_guardrails.py`; em `2026-05-10`, essa semantica foi recentralizada para recomendacao de Mesa (`nr35_mesa_recommended`/`high_risk_mesa_recommended`) sem bloqueio universal, mantendo o gate de qualidade como trava para ficha tecnica incompleta
- `validacao PR I`: `python -m py_compile web/app/domains/chat/high_risk_family_guardrails.py web/app/domains/chat/laudo_decision_services.py web/tests/test_high_risk_family_guardrails_pr_i.py`; `cd web && PYTHONPATH=. python -m pytest tests/test_high_risk_family_guardrails_pr_i.py tests/test_mobile_only_client_no_mesa_pr_f.py tests/test_company_no_mesa_multi_user_pr_g.py tests/test_company_with_mesa_premium_pr_h.py tests/test_mobile_chat_first_capability_aliases.py -q` (`14 passed`); recorte familias modeladas NR35 Ponto de Ancoragem, NR13 Caldeira, NR13 Vaso de Pressao, NR20 Prontuario e NR10 Prontuario (`5 passed`); recorte entitlement/finalizacao Chat sem Mesa/com Mesa (`5 passed`); `git diff --check`; `AMBIENTE=dev make release-verify-local`
...
### `PKT-MOBILE-CHAT-REISSUE-SIGNAL-01` - Sinal leve de reemissao recomendada no Chat/Mobile

- `status`: concluido localmente em `2026-04-27`; objetivo e expor o sinal leve de reemissao recomendada sem criar UI grande, sem tratar PDF operacional como oficial e sem mexer em release gate, mobile smoke, Maestro ou Android build nativo
- `checkpoint 2026-04-27`: o backend ja expunha `official_issue_summary` para lista mobile, SSR do Chat Inspetor e historico; o ajuste atual amplia o contrato para `reissue_recommended`, `issue_status`, `reissue_reason_codes` e `reissue_reason_summary`, cobrindo tambem nova aprovacao/snapshot sem depender exclusivamente de `primary_pdf_diverged`
- `checkpoint 2026-04-27`: o Mobile passa a interpretar reemissao recomendada por `reissue_recommended || primary_pdf_diverged` em home operacional, historico, configuracoes, exportacao e central de atividade; a UI continua sendo sinal/chip/alerta leve, sem tela nova
- `validacao executada`: `python -m py_compile web/app/shared/official_issue_package.py web/app/domains/chat/auth_mobile_support.py web/tests/test_v2_android_case_adapter.py`; `cd web && PYTHONPATH=. python -m pytest tests/test_v2_android_case_adapter.py::test_mobile_laudos_expoe_alerta_de_reemissao_do_pdf_oficial tests/test_v2_android_case_adapter.py::test_mobile_laudos_expoe_reemissao_recomendada_por_novo_snapshot_aprovado tests/test_v2_android_case_adapter.py::test_portal_inspetor_expoe_resumo_de_reemissao_no_contexto_ssr tests/test_official_issue_package.py::test_build_official_issue_package_expoe_emissao_ativa_e_reemissao_recomendada -q` (`4 passed`); `cd web && PYTHONPATH=. python -m pytest tests/test_official_issue_package.py tests/test_v2_android_case_adapter.py tests/test_inspection_entry_mode_phase_c_web.py -q` (`17 passed`); `cd web && PYTHONPATH=. python -m pytest tests/test_inspection_entry_mode_phase_c_web.py::test_portal_app_ssr_expone_governanca_de_reemissao_no_ponto_de_entrada tests/test_mobile_chat_first_capability_aliases.py -q` (`6 passed`); `cd android && npm test -- --runInBand src/features/chat/buildThreadContextState.test.ts src/features/common/buildInspectorBaseDerivedStateSections.test.ts src/features/history/HistoryDrawerPanel.test.tsx src/features/activity/activityNotificationHelpers.test.ts src/features/activity/useActivityCenterController.test.ts src/features/settings/exportDataFlow.test.ts` (`56 passed`); `cd android && npm run typecheck`; `cd android && npm run lint`; `cd android && npm run format:check`; `git diff --check`; `AMBIENTE=dev make release-verify-local`

### `PKT-MOBILE-ONLY-CLIENT-NO-MESA-01` - Cliente individual/mobile-only sem Mesa

- `status`: concluido localmente em `2026-04-27`; objetivo e provar que Mesa nao e universal para familias simples, sem relaxar NR35, sem criar emissao oficial sem capability/signatario e sem chamar PDF operacional de oficial
- `checkpoint 2026-04-27`: criado `web/tests/test_mobile_only_client_no_mesa_pr_f.py`, com fixture de tenant `inspector_chat` + `mobile_single_operator`, portais `cliente`/`inspetor`, sem `revisor`, com `mobile_case_approve`/`case_self_review`, `case_create` e `case_finalize_request`, e sem `reviewer_decision`/`reviewer_issue`/`official_issue_create`
- `checkpoint 2026-04-27`: o teste positivo usa familia simples `padrao`; finalizacao pelo Chat Inspetor cai em revisao interna governada (`mobile_autonomous` legado com `tenant_without_mesa`), grava `ApprovedCaseSnapshot`, preserva PDF operacional como `nome_arquivo_pdf`, nao cria `EmissaoOficialLaudo`, faz o Portal Cliente mostrar Mobile/Chat/self-review liberados e Mesa/emissao oficial nao incluidas, e faz o Mobile nao expor `official_issue_summary`
- `checkpoint 2026-04-27`: o teste negativo usa NR35 Linha de Vida no mesmo tenant e valida bloqueio `nr35_mesa_required_unavailable`, mantendo o caso em rascunho sem snapshot aprovado e sem emissao oficial
- `validacao executada`: `python -m py_compile web/tests/test_mobile_only_client_no_mesa_pr_f.py`; `cd web && PYTHONPATH=. python -m pytest tests/test_mobile_only_client_no_mesa_pr_f.py tests/test_mobile_chat_first_capability_aliases.py -q` (`7 passed`); `cd web && PYTHONPATH=. python -m pytest tests/test_tenant_entitlements_critical.py::test_finalizacao_sem_mesa_contratada_aprova_no_fluxo_interno_governado tests/test_tenant_entitlements_critical.py::test_previa_finalizacao_sem_mesa_orienta_resolver_correcoes_abertas tests/test_tenant_entitlements_critical.py::test_previa_finalizacao_sem_mesa_expoe_revisao_interna_sem_rotulo_mesa tests/test_tenant_entitlements_critical.py::test_previa_finalizacao_nr35_sem_mesa_continua_bloqueada_por_familia tests/test_tenant_entitlements_critical.py::test_inspetor_sem_servicos_da_mesa_nao_abre_preparacao_de_emissao -q` (`5 passed`); `cd web && PYTHONPATH=. python -m pytest tests/test_cliente_portal_critico.py::test_admin_cliente_bootstrap_explica_recursos_do_pacote_sem_mesa -q` (`1 passed`); `git diff --check`; `AMBIENTE=dev make release-verify-local`

### `PKT-COMPANY-NO-MESA-MULTI-USER-01` - Empresa sem Mesa com multiplos usuarios

- `status`: concluido localmente em `2026-04-27`; objetivo e provar empresa `inspector_chat` sem Mesa no modelo `standard`, com Admin Cliente gerenciando equipe, multiplos inspetores operando por Chat/Mobile e self-review governado apenas para familias simples
- `checkpoint 2026-04-27`: criado `web/tests/test_company_no_mesa_multi_user_pr_g.py`; a fixture cria empresa sem usuario revisor, Admin Cliente tenant-scoped e inspetor base, depois o Admin Cliente cria segundo inspetor pelo proprio endpoint `/cliente/api/usuarios`
- `checkpoint 2026-04-27`: o teste positivo cobre pacote/capabilities no Portal Cliente, bloqueio de criacao de Admin Cliente extra, bloqueio de mudanca direta de plano, bloqueio de concessao do portal `revisor`, criacao de caso `padrao` pelo Inspetor, finalizacao interna governada sem handoff para Mesa, `ApprovedCaseSnapshot`, ausencia de `EmissaoOficialLaudo`, PDF operacional apenas em `nome_arquivo_pdf`, Mobile sem `official_issue_summary` e isolamento entre inspetores da mesma empresa
- `checkpoint 2026-04-27`: o teste negativo cobre NR35 Linha de Vida no mesmo modelo sem Mesa, mantendo bloqueio `nr35_mesa_required_unavailable`, caso em rascunho, sem snapshot aprovado e sem emissao oficial
- `validacao executada`: `python -m py_compile web/tests/test_company_no_mesa_multi_user_pr_g.py`; `cd web && PYTHONPATH=. python -m pytest tests/test_company_no_mesa_multi_user_pr_g.py -q` (`2 passed`); `cd web && PYTHONPATH=. python -m pytest tests/test_company_no_mesa_multi_user_pr_g.py tests/test_mobile_only_client_no_mesa_pr_f.py tests/test_mobile_chat_first_capability_aliases.py -q` (`9 passed`); recorte sem Mesa de `tests/test_tenant_entitlements_critical.py` (`5 passed`); recorte Portal Cliente de pacote/equipe/plano/limite operacional (`4 passed`); `git diff --check`; `AMBIENTE=dev make release-verify-local`

### `PKT-COMPANY-WITH-MESA-PREMIUM-01` - Empresa com Mesa / premium intacta

- `status`: concluido localmente em `2026-04-27`; objetivo e provar que o fluxo premium com Mesa segue intacto depois dos cenarios sem Mesa dos PRs F/G, sem relaxar NR35 e sem tratar PDF operacional como emissao oficial
- `checkpoint 2026-04-27`: criado `web/tests/test_company_with_mesa_premium_pr_h.py`; a fixture usa pacote `inspector_chat_mesa`, `operating_model=standard`, portais `cliente`/`inspetor`/`revisor`, familia simples modelada `cbmgo`, signatario governado compativel e storage oficial isolado por `tmp_path`
- `checkpoint 2026-04-27`: o teste positivo cobre Portal Cliente com Mesa/emissao/signatario incluidos, Mobile/Chat sem capacidade de emissao pelo inspetor, criacao/coleta/finalizacao pelo Chat Inspetor, handoff `send_to_mesa`, aprovacao por Mesa, `ApprovedCaseSnapshot`, PDF operacional pre-emissao sem `issue_number`, emissao oficial governada, pacote ZIP congelado, download/auditoria e Portal Cliente/Mobile apontando para a emissao ativa
- `validacao executada`: `python -m py_compile web/tests/test_company_with_mesa_premium_pr_h.py`; `cd web && PYTHONPATH=. python -m pytest tests/test_company_with_mesa_premium_pr_h.py -q` (`1 passed`); `cd web && PYTHONPATH=. python -m pytest tests/test_company_with_mesa_premium_pr_h.py tests/test_company_no_mesa_multi_user_pr_g.py tests/test_mobile_only_client_no_mesa_pr_f.py tests/test_mobile_chat_first_capability_aliases.py -q` (`10 passed`); recorte com Mesa de `tests/test_tenant_entitlements_critical.py` (`3 passed`); recorte Portal Cliente pacote/documentos (`4 passed`); recorte emissao oficial/reemissao em `tests/test_regras_rotas_criticas.py` (`2 passed`); `git diff --check`; `AMBIENTE=dev make release-verify-local`
...
- `checkpoint 2026-04-26`: politica historica do piloto mantida em `mesa_required`; em `2026-05-10`, a regra de familia alto risco foi recentralizada para recomendacao de Mesa sem bloqueio universal, preservando blockers de campos, fotos, conclusao e rastreabilidade minima
...
- `checkpoint 2026-04-25`: verificada a composição real de `release-gate-real` no `Makefile` antes da mudança; introduzidos `document-contract-check`, `release-verify-local` e `release-verify`, mantendo `release-gate-hosted`, `release-gate-real` e `release-gate` sem mudança de papel funcional; `document-contract-check` passou a falhar cedo quando `pypdf` não estiver disponível no Python ativo do workspace web, em vez de depender do `skip` interno do teste para sinalizar cobertura documental
- `checkpoint 2026-04-25`: documentação operacional alinhada em `AGENTS.md`, `README.md`, `docs/STATUS_CANONICO.md` e `docs/developer-experience/04_mobile_smoke_and_release_gate.md`, deixando explícita a leitura entre `make verify`, `make release-verify-local` e `make release-verify`
- `validacao executada`: `git diff --check`, `make verify` e `make release-verify-local`, todos verdes no ambiente local desta cópia; `release-verify` não foi disparado nesta rodada porque reaproveita a lane real pré-deploy (`release-gate-real`) com E2E hospedada e smoke mobile real, mais cara e dependente do ambiente operacional completo
- `checkpoint 2026-04-25`: rodada controlada do gate completo executada; `release-gate-hosted` passou dentro de `make release-verify`, com `mesa-acceptance`, `document-acceptance` e `observability-acceptance` verdes, e `uploads-cleanup-check` também passou isoladamente; a quebra real apareceu apenas em `smoke-mobile`, quando `scripts/run_mobile_pilot_runner.py` tentou ler `android/android/app/build.gradle` e abortou com `FileNotFoundError`, revelando que a lane real ainda depende do projeto nativo gerado via `android:prebuild` ou de um tratamento explícito desse pré-requisito no próprio runner
- `checkpoint 2026-04-25`: criada a matriz [docs/RELEASE_GATE_READINESS.md](docs/RELEASE_GATE_READINESS.md) com diferenças entre `make verify`, `make release-verify-local` e `make release-verify`, pré-requisitos do gate completo, variáveis/serviços dependentes de infra e classificação atual entre bloqueadores reais (`smoke-mobile` sem `android/android`) e pendências aceitáveis com ressalva (`REDIS_URL` ausente, pacote externo `../Tarie 2/schemas` ausente e vulnerabilidades `moderate` transitivas do Expo fora do threshold atual)
- `checkpoint 2026-04-25`: a lane mobile real foi endurecida sem mexer em lógica funcional do app; entrou o target `mobile-native-preflight`, que executa `cd android && npm run android:prebuild` apenas quando `android/android/app/build.gradle` estiver ausente, gravando estado em `.tmp_online/devkit/mobile_native_preflight_status.json` e `artifacts/mobile_native_preflight/<timestamp>/preflight_status.json`; `mobile-acceptance` e `smoke-mobile` passaram a depender desse preflight, e o runner `scripts/run_mobile_pilot_runner.py` agora copia o estado do preflight para o artefato da lane e falha com mensagem operacional clara se for chamado diretamente sem projeto nativo materializado
- `checkpoint 2026-04-25`: validado `make smoke-mobile` apos o endurecimento; o `FileNotFoundError` cru deixou de ser o primeiro bloqueador, o preflight foi necessario (`native_project_present_before=false`) e executou `cd android && npm run android:prebuild`, mas a rodada parou no proprio `expo prebuild` com falha `Cannot find module '@expo/image-utils'`; a classificacao atual do novo bloqueio e `instalacao/build Android`, nao `frontend`, `Redis` ou `schema V2`
- `checkpoint 2026-04-25`: investigado o bloqueio `Cannot find module '@expo/image-utils'`; `npm ci` nao alterou manifesto, `npx expo install --check` passou, o CLI usado e local (`node_modules/.bin/expo`), e quem importa `@expo/image-utils` e o stack Expo (`@expo/prebuild-config`, `@expo/cli`, `expo-asset`, `expo-notifications`); a causa provavel ficou em Node ativo `v22.12.0`, pois o mesmo prebuild passou com Node `v20.20.2` e `v22.22.2`
- `checkpoint 2026-04-25`: o workspace Android passou a declarar engines `node: ^20.19.0 || ^22.13.0 || >=24`; os scripts `android:prebuild` e `android:preview:fresh` usam `npx --no-install expo prebuild`; `mobile-native-preflight` registra Node/npm no artefato, reexecuta prebuild quando o estado anterior era falho e usa uma versao Node compativel via `nvm` quando a versao ativa nao atende ao requisito
- `validacao executada`: `git diff --check`, `make release-verify-local` e `make smoke-mobile`; `release-verify-local` passou, `smoke-mobile` avancou por preflight, emulador e bootstrap da API local, mas parou em `no such table: usuarios`; novo bloqueador classificado como `backend local`/schema do banco da API mobile, deixando de ser problema de `@expo/image-utils`, CLI global, lockfile ou Expo vulnerability
- `checkpoint 2026-04-25`: o banco local do `smoke-mobile` foi tornado deterministico e auditavel sem alterar startup de producao; o runner cria `artifacts/mobile_pilot_run/<timestamp>/mobile_local.sqlite3`, aplica `app.shared.database.inicializar_banco()` com migrations e `SEED_DEV_BOOTSTRAP=1`, roda `web/scripts/seed_mobile_pilot_data.py` no mesmo `DATABASE_URL` e registra `mobile_database_preflight.json`; a validacao isolada de schema/seed passou e a rodada `make smoke-mobile` deixou de falhar em `usuarios`, avancando ate o novo bloqueador `HTTP Error 404: Not Found` na rota admin de observabilidade mobile com `TARIEL_V2_ANDROID_ROLLOUT_OBSERVABILITY` desabilitada
- `checkpoint 2026-04-25`: o backend local da lane mobile passou a ficar deterministico tambem para rollout/admin operator-run sem mexer em producao; `scripts/run_mobile_pilot_runner.py` agora resolve o tenant demo diretamente do banco local/seed oficial, injeta no ambiente da API local apenas as flags `TARIEL_V2_ANDROID_PUBLIC_CONTRACT`, `TARIEL_V2_ANDROID_ROLLOUT`, `TARIEL_V2_ANDROID_ROLLOUT_OBSERVABILITY`, `TARIEL_V2_ANDROID_PILOT_TENANT_KEY`, `TARIEL_V2_ANDROID_ROLLOUT_STATE_OVERRIDES`, `TARIEL_V2_ANDROID_ROLLOUT_SURFACE_STATE_OVERRIDES`, `TARIEL_V2_ANDROID_FEED_ENABLED` e `TARIEL_V2_ANDROID_THREAD_ENABLED`, e registra `backend_env_flags_applied.json`, `mobile_pilot_seed_payload.json` e `operator_route_preflight.json`
- `checkpoint 2026-04-25`: validado o novo preflight explicito das rotas admin logo apos subir a API local e autenticar no admin; na rodada desta copia, o tenant resolvido pela seed foi `1` (`Empresa Demo (DEV)`), `operator_route_preflight_status=ok`, `GET /admin/api/mobile-v2-rollout/summary` respondeu `200` e a lane avancou para build/install do APK e flow Maestro, eliminando o `404` como bloqueador atual
- `checkpoint 2026-04-25`: depois da correcao do backend local, `make smoke-mobile` passou a falhar em `target_tapped_but_callback_not_fired`, com `v2_served_total_after=2`, operator run abortado por ausencia de `human_ack` e sem sinais ambientais de emulador; classificacao atual do novo bloqueio: `bug funcional mobile`, nao `API route`, `backend local`, `instalacao APK` ou `ADB/emulador`
- `validacao executada`: `python3 -m py_compile scripts/run_mobile_pilot_runner.py`, `git diff --check`, `make release-verify-local` e `make smoke-mobile`; `release-verify-local` passou, `smoke-mobile` nao voltou a `404` e expôs o novo bloqueador funcional do app, entao `make release-verify` nao foi executado nesta rodada
- `checkpoint 2026-04-25`: investigado o `target_tapped_but_callback_not_fired` nos artefatos `artifacts/mobile_pilot_run/20260425_172204`; o alvo tocado foi `history-item-1`, a tela mudou para `chat-thread-surface` com o texto `Mobile pilot V2 target`, mas o flow esperava marcadores internos `history-selection-callback-*` ausentes no APK reaproveitado; o flow passou a confirmar a selecao por UI real e o runner passou a registrar `operator_run_action_diagnostics.json`, sem sintetizar `human_ack` pelo host quando o Maestro passa
- `checkpoint 2026-04-25`: a rodada posterior `artifacts/mobile_pilot_run/20260425_181246` eliminou o bloqueio inicial e provou request real de `human_ack` para `feed` (`status=accepted`, target `1`), mas o flow ainda falhou antes do ack de `thread`: depois de fechar a central de atividade, `mesa-tab-button` nao existia na UI final, enquanto `chat-report-pack-card-open-mesa`/`Abrir Mesa` estava visivel; classificacao atual do novo bloqueio: `selector Maestro` para entrada na Mesa/thread, nao backend callback, auth/tenant, API contract, ADB/emulador ou rota admin
- `validacao executada`: `python3 -m py_compile scripts/run_mobile_pilot_runner.py`, `cd android && npm test -- --runInBand src/features/common/usePilotAutomationController.test.ts src/config/mobileV2HumanValidation.test.ts`, `git diff --check`, `make release-verify-local` e `make smoke-mobile`; `release-verify-local` passou duas vezes, `smoke-mobile` avancou ate `feed_only_confirmed` e `make release-verify` nao foi executado porque a lane mobile real ainda nao passou
- `checkpoint 2026-04-25`: o flow Maestro deixou de depender do selector obsoleto `mesa-tab-button` e passou a entrar na Mesa pelo CTA real `chat-report-pack-card-open-mesa`/`Abrir Mesa`, aguardando `mesa-thread-surface` e `mesa-thread-loaded`; o runner passou a registrar `mesa_entry_target_found`, `mesa_entry_target_tapped`, `thread_surface_confirmed`, `thread_ack_request_observed` e `thread_ack_status`; a rodada `artifacts/mobile_pilot_run/20260425_184325` confirmou que o CTA foi encontrado/tocado e que a superfície da Mesa abriu, mas a tela ficou em `mesa-thread-blocked` com “Mesa indisponível para esta conta”, sem request real de `human_ack` para `thread`; nova classificação: `app mobile UI state`/`auth-tenant`, não `selector Maestro`
- `validacao executada`: `python3 -m py_compile scripts/run_mobile_pilot_runner.py`, `cd android && npm test -- --runInBand src/features/chat/ThreadConversationPane.test.tsx src/features/common/usePilotAutomationController.test.ts src/config/mobileV2HumanValidation.test.ts`, `git diff --check`, `make release-verify-local` e `make smoke-mobile`; `release-verify-local` passou, `smoke-mobile` parou em `feed_only_confirmed` por Mesa bloqueada no app, e `make release-verify` nao foi executado porque a lane real nao fechou
- `checkpoint 2026-04-25`: raiz do `mesa-thread-blocked` isolada no app mobile: `ThreadConversationMesaSurface` bloqueava quando `mesaAcessoPermitido=false`, e `buildInspectorConversationDerivedState` calculava esse estado apenas por portal efetivo `revisor`; a conta real do smoke (`inspetor@tariel.ia`) usa `allowed_portals=["inspetor"]`, mas o tenant/pacote `inspector_chat_mesa` expõe `tenant_access_policy.user_capability_entitlements.inspector_send_to_mesa=true`; o app agora lê capabilities efetivas do usuário e libera a superfície da Mesa quando há `revisor` ou `inspector_send_to_mesa`, sem alterar seed, flags, rollout ou backend
- `validacao executada`: `cd android && npm test -- --runInBand src/features/common/mobileUserAccess.test.ts src/features/common/buildInspectorBaseDerivedStateSections.test.ts src/features/chat/ThreadConversationPane.test.tsx src/features/common/usePilotAutomationController.test.ts src/config/mobileV2HumanValidation.test.ts`, `cd android && npx prettier --check src/features/common/mobileUserAccess.ts src/features/common/mobileUserAccess.test.ts src/features/common/buildInspectorBaseDerivedStateSections.ts src/features/common/buildInspectorBaseDerivedStateSections.test.ts`, `git diff --check` e `make release-verify-local`; todos passaram; a rodada `make smoke-mobile` em `artifacts/mobile_pilot_run/20260425_191651` ainda parou em `feed_only_confirmed`, mas o artefato mostra `npm run android:preview` falhando em `Cannot find module '@expo/metro/metro-config'` e o runner reaproveitando APK antigo, então a nova classificação operacional é `instalacao/build Android`/APK stale fora do escopo desta correção
- `checkpoint 2026-04-25`: investigado o erro `Cannot find module '@expo/metro/metro-config'`; nenhum arquivo local importava esse caminho, a importação vinha do Expo CLI (`expo/node_modules/@expo/cli/.../instantiateMetro.js`), `expo/metro-config` e `@expo/metro/metro-config` resolviam após `npm ci`, e o build só ficou determinístico ao rodar com Node compatível (`v22.22.2`) e estado limpo do preview; o runner agora seleciona Node compatível via `nvm`, passa `NODE_BINARY` ao preview, registra `apk_build_preflight.json` e falha antes do Maestro se `android:preview` não gerar APK novo, salvo opt-in explícito `MOBILE_ALLOW_STALE_APK=1`
- `validacao executada`: `cd android && npm ci`, `cd android && npx expo install --check`, `cd android && nvm use v22.22.2 && npm run android:prebuild`, `cd android && nvm use v22.22.2 && ANDROID_PREVIEW_CLEAN=1 npm run android:preview`, testes mobile focais, `python3 -m py_compile scripts/run_mobile_pilot_runner.py scripts/run_mobile_native_preflight.py`, `cd android && npx prettier --check scripts/run-android-preview.cjs`, `git diff --check`, `make release-verify-local` e `make smoke-mobile`; `release-verify-local` passou, `smoke-mobile` gerou/instalou APK novo em `artifacts/mobile_pilot_run/20260425_194938` (`stale_apk_detected=false`) e avançou até novo bloqueador de login/conectividade (`Network request failed`, `ui_login_status_api=offline`, `app_opened_but_target_not_reached`), classificado como `rede/reverse proxy`; `make release-verify` não foi executado porque a lane mobile real ainda não passou
- `checkpoint 2026-04-25`: a conectividade do APK preview com a API local ficou determinística sem mexer em lógica de produto; o runner passou a compilar o APK da lane com `EXPO_PUBLIC_API_BASE_URL=http://127.0.0.1:8000`, `EXPO_PUBLIC_AUTH_WEB_BASE_URL=http://127.0.0.1:8000` e `EXPO_PUBLIC_ANDROID_LOCALHOST_STRATEGY=reverse`, preservando `127.0.0.1` no app apenas quando essa env build-time está explícita; `android:preview` habilita cleartext HTTP temporariamente no manifest do build local e restaura o manifest fonte antes de sair; o runner registra `mobile_network_preflight.json` com health do host, lista de `adb reverse`, device serial, probes device e cleartext do APK antes de abrir o app
- `validacao executada`: testes focais de config mobile, `python3 -m py_compile scripts/run_mobile_pilot_runner.py scripts/run_mobile_native_preflight.py`, `node --check android/scripts/run-android-preview.cjs`, `cd android && npx prettier --check scripts/run-android-preview.cjs src/config/apiCore.ts src/config/apiCore.test.ts`, `git diff --check`, `make release-verify-local` duas vezes e `make smoke-mobile`; a primeira rodada de smoke abortou antes do Maestro provando que `10.0.2.2` nao alcançava o backend local, e a rodada seguinte `artifacts/mobile_pilot_run/20260425_203912` passou o preflight de rede (`backend_health_host_ok=true`, `adb_reverse_configured=true`, `cleartext_http_allowed=true`) e entrou no Maestro, eliminando `Network request failed`/`ui_login_status_api=offline`; o novo bloqueador é `thread_opened_but_no_human_ack`, com thread/Mesa visível e sem request real de ack, classificado como `API contract`/flag build-time V2 do app, nao `rede/reverse proxy`
- `checkpoint 2026-04-25`: raiz do `thread_opened_but_no_human_ack` isolada na flag build-time do APK: o app calcula `ui_runtime_flag_enabled` em `usePilotAutomationController` via `getAndroidV2ReadContractsRuntimeSnapshot()`, que lê `EXPO_PUBLIC_ANDROID_V2_READ_CONTRACTS_ENABLED`; a rodada `20260425_203912` mostrou `Usando Android V2 preview flag=<unset>`, `ui_runtime_flag_enabled=false`, `runtime_flag_raw_value=none`, request trace `not_created` e apenas tráfego legacy de feed, portanto o controller não recebeu metadata V2/organic validation para disparar `human_ack` de `thread`; o runner agora compila a lane local com `EXPO_PUBLIC_ANDROID_V2_READ_CONTRACTS_ENABLED=1`, registra `android_v2_read_contracts_enabled_for_build` e `build_env_flags_applied` em `apk_build_preflight.json`, e `android:preview` propaga explicitamente essa env ao Gradle
- `validacao executada`: `cd android && npm test -- --runInBand src/config/mobileV2Config.test.ts src/config/mobileV2Rollout.test.ts src/config/mobileV2HumanValidation.test.ts src/config/mesaApi.test.ts src/features/common/usePilotAutomationController.test.ts`, `python3 -m py_compile scripts/run_mobile_pilot_runner.py scripts/run_mobile_native_preflight.py`, `node --check android/scripts/run-android-preview.cjs`, `cd android && npx prettier --check scripts/run-android-preview.cjs src/config/mobileV2Config.test.ts`, `git diff --check`, `make release-verify-local` e `make smoke-mobile`; `release-verify-local` passou completo, e `smoke-mobile` gerou APK novo em `artifacts/mobile_pilot_run/20260425_210458` com `EXPO_PUBLIC_ANDROID_V2_READ_CONTRACTS_ENABLED=1`, `ui_runtime_flag_enabled=true`, `v2_served_total_after=4`, `human_ack` real aceito para `feed`, mas ainda parou em `feed_only_confirmed` porque nao houve request real de `human_ack` para `thread` mesmo com `thread` V2 servida e `mesa-thread-loaded` visivel; novo bloqueador classificado como `app mobile UI state`/wiring do ack de thread, nao `flag build-time mobile`
- `checkpoint 2026-04-25`: raiz do `feed_only_confirmed` final isolada no caminho da central de atividade: `monitorActivityFlow` carregava as mensagens V2 da thread do laudo ativo e marcava a Mesa como carregada, mas descartava a metadata de leitura/organic validation antes de atualizar `ultimoMetaLeituraThreadMesa`; como `laudoMesaCarregado` ja estava preenchido quando o usuario abria a Mesa, o controller nao recarregava a thread, `usePilotAutomationController` via `mesaThreadRenderConfirmada` enxergava a superficie visivel sem metadata ackable e nao criava request de `human_ack` para `surface=thread`; o fluxo agora preserva `extractMobileV2ReadRenderMetadata(payload)` no caminho monitor -> activity center -> root local state, sem ack sintetico e sem alterar backend/seed/flags
- `validacao executada`: testes mobile focais de config, dedupe, controller, activity center, thread e adapter, `python3 -m py_compile scripts/run_mobile_pilot_runner.py scripts/run_mobile_native_preflight.py`, `node --check android/scripts/run-android-preview.cjs`, `git diff --check`, `make release-verify-local`, `make smoke-mobile` e `make release-verify`; `smoke-mobile` passou em `artifacts/mobile_pilot_run/20260425_213227`, e a promocao completa repetiu o sucesso em `artifacts/mobile_pilot_run/20260425_214252`, com `feed_covered=True`, `thread_covered=True`, `operator_run_outcome=completed_successfully`, `human_ack_recent_events_after=2`, `thread_ack_dedupe_key=orgv_440141257fa1:thread:rendered:1` e `thread_ack_status=accepted`
- `checkpoint final 2026-04-25`: fechamento pos-green sem nova logica de produto; [docs/RELEASE_GATE_READINESS.md](docs/RELEASE_GATE_READINESS.md) agora registra o estado verde atual do gate completo, os artefatos finais `artifacts/mobile_pilot_run/20260425_213227` e `artifacts/mobile_pilot_run/20260425_214252`, e os bloqueadores corrigidos desde o inicio da normalizacao; estado consolidado: `make release-verify-local`, `make smoke-mobile` e `make release-verify` passaram nesta copia local
- `checkpoint pos-green 2026-04-25`: refatoracao mecanica do `scripts/run_mobile_pilot_runner.py` concluida sem alterar comportamento, extraindo a orquestracao final em fases nomeadas de ambiente/artefatos, backend local, operator-run, preparo do APK/Maestro, coleta de evidencias e diagnostico final; validacao executada com `python3 -m py_compile scripts/run_mobile_pilot_runner.py scripts/run_mobile_native_preflight.py`, `git diff --check`, `make release-verify-local`, `make smoke-mobile` (`artifacts/mobile_pilot_run/20260425_221715`) e `make release-verify` (`artifacts/mobile_pilot_run/20260425_223136`), ambos com `result=success_human_confirmed`, `feed_covered=True`, `thread_covered=True` e `human_ack_recent_events_after=2`; uma primeira tentativa de `make release-verify` falhou transientemente no `mesa-acceptance` por timeout Playwright ao ler CSRF, e o teste alvo passou isolado antes da reexecucao completa verde
- `checkpoint reprodutibilidade 2026-04-25`: fechado o bloqueador de checkout limpo do `hygiene-check`; `web/artifacts/.gitignore` e `web/artifacts/README.md` passaram a ser versionados enquanto `web/artifacts/*` continua ignorando payloads gerados, e os pre-requisitos de host/`AMBIENTE=dev` foram documentados em `README.md` e [docs/RELEASE_GATE_READINESS.md](docs/RELEASE_GATE_READINESS.md); validacao com `git diff --check`, `AMBIENTE=dev make release-verify-local` no worktree normal, e em worktree limpo com patch aplicado: `git status --short`, `git ls-files --others --exclude-standard`, `AMBIENTE=dev make release-verify-local` e `AMBIENTE=dev make release-verify`, com smoke final em `artifacts/mobile_pilot_run/20260425_232916` (`result=success_human_confirmed`, `native_prebuild_executed=True`, `feed_covered=True`, `thread_covered=True`, `human_ack_recent_events_after=2`)

### `PKT-INSPETOR-BACKEND-UX-01` - Backlog objetivo da próxima sequência estrutural

- `status`: documentado em `2026-04-24`; consolidada a ordem de execução para estabilizar o `inspetor`, simplificar a jornada principal, endurecer a fronteira transacional do backend e só depois continuar a limpeza de hotspots, rendering manual e observabilidade fina
- `checkpoint 2026-04-24`: criado o plano [docs/restructuring-roadmap/138_backlog_execucao_inspetor_backend_ux.md](docs/restructuring-roadmap/138_backlog_execucao_inspetor_backend_ux.md), transformando a leitura executiva em backlog acionável por `3` fases: `Fase 1` fecha estado canônico + jornada principal do inspetor, `Fase 2` fecha transações + contrato de evidência/gate, e `Fase 3` retoma hotspots + `innerHTML` residual + performance/observabilidade; a frente puxa contexto de `MP-007`, `docs/restructuring-roadmap/136_plano_tecnico_inspetor_mesa_flagship.md`, `docs/restructuring-roadmap/133_inspetor_correcoes_sem_mesa.md`, `web/docs/correcao_sem_mesa.md` e `docs/STATUS_CANONICO.md` sem reabrir o contrato de produto do `inspetor + mesa`
- `checkpoint 2026-04-24`: `Lote 1.1` iniciado e fechado localmente; `chat_index_page.js` deixou de carregar diretamente o bloco grande de bindings/reconciliação do estado do inspetor, que foi centralizado em `workspace_runtime_state.js` por `criarBindingsEstadoInspector`; o SSR de `/app/` sem query voltou a limpar o contexto ativo e abrir a home como tela ativa, preservando `?laudo=` e `?home=1`; validação local com `node --check` nos JS alterados, `py_compile` em `session_helpers.py`, testes focais de autoridade do inspetor/boot, smoke parcial, `git diff --check`, `make verify` e `make hygiene-check`
- `checkpoint 2026-04-24`: `Lote 1.2` iniciado e fechado localmente; a entrada do inspetor ganhou CTA primário de `Nova inspeção` no landing, o rodapé deixou de expor uma faixa redundante de ações no primeiro passo, `Anexar`/`Foto` ficaram como gatilhos visíveis do próprio composer com botões legados escondidos para compatibilidade, e `Mesa`/`Áudio` viraram ações compactas do composer quando o estado permitir; validação local com `node --check`, smoke parcial, testes focais de autoridade do inspetor, `git diff --check`, `make verify` e `make hygiene-check`
- `checkpoint 2026-04-24`: `Lote 2.1` iniciado com os fluxos principais do Chat Inspetor; `iniciar`, `finalizar` e `reabrir` agora usam `commit_ou_rollback_operacional` explicitamente nos command services, sem `banco.commit()` direto e sem depender do commit implícito de `obter_banco()` nesses caminhos; `tests/test_transaction_contract.py` passou a travar esse contrato; validação focal com `py_compile`, `tests/test_transaction_contract.py` e recorte de `tests/test_regras_rotas_criticas.py` para iniciar/finalizar/reabrir, além de `git diff --check`, `make verify` e `make hygiene-check`
- `checkpoint 2026-04-24`: `Lote 2.1` avançou para Mesa, Pendências e Correções; `mesa_message_routes.py` deixou de usar `banco.commit()` direto nos envios do inspetor para a Mesa por meio de `commit_ou_rollback_operacional_preservando_integridade`, preservando replay idempotente; `pendencias.py` e `corrections.py` passaram a comitar explicitamente suas mutações; `tests/test_transaction_contract.py` passou a cobrir esses contratos; validação focal com `py_compile`, `ruff`, contrato transacional, idempotência da Mesa, anexos/respostas da Mesa, pendências e correções estruturadas
- `checkpoint 2026-04-24`: `Lote 2.2` iniciou o contrato tipado de evidência; criado `evidence_contract.py` com `EvidenceClassification`, tipos (`text`, `photo`, `document`) e origens (`message`, `visual_learning`), e `gate_helpers.py` passou a calcular texto/foto/documento/evidência consolidada por esse contrato sem alterar a compatibilidade de placeholders; `web/docs/checklist_qualidade.md` e `tests/test_evidence_contract.py` foram atualizados; validação focal com `py_compile`, `ruff`, `tests/test_evidence_contract.py`, recortes de `test_semantic_report_pack_catalog_fallback.py` e `test_regras_rotas_criticas.py` para gate/finalização
- `checkpoint 2026-04-24`: `Lote 2.2` avançou para anexos reais; `EvidenceClassification` passou a carregar `attachment_id`, `mime_type`, `raw_category` e elegibilidade, `classificar_anexo_mesa_evidencia(...)` classifica `AnexoMesa` por categoria/mime, e `gate_helpers.py` agora soma anexos reais carregados por `selectinload`; `[ANEXO_MESA_SEM_TEXTO]` deixou de contar como texto, e o teste crítico cobre finalização aprovada com foto real anexada
...
- `checkpoint 2026-04-24`: novo slice frontend do Portal Admin Cliente concluído localmente; o bloco `empresa-resumo-detalhado` em `web/static/js/cliente/portal_admin_surface.js` deixou de interpolar HTML com dados do tenant e passou a montar status, consumo mensal, chips de capacidade, contexto e guidance por DOM seguro (`textContent`, `dataset` e helpers pequenos de chip), preservando o contrato visual atual; validação local com `node --check web/static/js/cliente/portal_admin_surface.js`, `cd web && PYTHONPATH=. python -m pytest tests/test_smoke.py::test_templates_cliente_explicitam_abas_e_formularios_principais -q`, `git diff --check`, `make web-ci`, `make production-ops-check-strict`, `make uploads-restore-drill`, `make hygiene-check` e `make verify`
- `checkpoint 2026-04-24`: novo slice frontend do Portal Admin Cliente concluído localmente; `renderOnboardingEquipe` em `web/static/js/cliente/portal_admin_surface.js` deixou de interpolar HTML com nomes/e-mails/status de usuários e passou a montar métricas, estado vazio, pendências e ações rápidas por DOM seguro, preservando os `data-act` de filtros, reset, bloqueio e abertura de cadastro; validação local com `node --check web/static/js/cliente/portal_admin_surface.js`, `cd web && PYTHONPATH=. python -m pytest tests/test_smoke.py::test_templates_cliente_explicitam_abas_e_formularios_principais -q`, `git diff --check`, `make web-ci`, `make production-ops-check-strict`, `make uploads-restore-drill`, `make hygiene-check` e `make verify`
- `checkpoint 2026-04-24`: novo slice frontend do Portal Admin Cliente concluído localmente; `renderHistoricoPlanos` e `renderPreviewPlano` em `web/static/js/cliente/portal_admin_surface.js` deixaram de interpolar HTML com plano, ator, data, impacto e limites comerciais e passaram a montar cards, chips, guidance e botão `registrar-interesse-plano` por DOM seguro, preservando `data-act`, `data-origem` e `data-plano`; validação local com `node --check web/static/js/cliente/portal_admin_surface.js`, `cd web && PYTHONPATH=. python -m pytest tests/test_smoke.py::test_templates_cliente_explicitam_abas_e_formularios_principais -q`, `git diff --check`, `make web-ci`, `make production-ops-check-strict`, `make uploads-restore-drill`, `make hygiene-check` e `make verify`
- `checkpoint 2026-04-24`: novo slice frontend do Portal Admin Cliente concluído localmente; `renderAdminAuditoria` em `web/static/js/cliente/portal_admin_surface.js` deixou de interpolar HTML com resumo, filtro, busca, protocolo, alvo, contexto e detalhes e passou a montar cards, filtros `data-audit-filter`, estados vazios e timeline por DOM seguro; validação local com `node --check web/static/js/cliente/portal_admin_surface.js`, `cd web && PYTHONPATH=. python -m pytest tests/test_smoke.py::test_templates_cliente_explicitam_abas_e_formularios_principais -q`, `git diff --check`, `make web-ci`, `make production-ops-check-strict`, `make uploads-restore-drill`, `make hygiene-check` e `make verify`
- `checkpoint 2026-04-24`: novo slice frontend do Portal Admin Cliente concluído localmente; `renderUsuarios` em `web/static/js/cliente/portal_admin_surface.js` deixou de interpolar HTML com usuários, editor, permissões por portal e ações administrativas, e o smoke passou a travar o contrato DOM por `act`/`dataset` em vez de HTML literal; validação local com `node --check web/static/js/cliente/portal_admin_surface.js`, `cd web && PYTHONPATH=. python -m pytest tests/test_smoke.py::test_templates_cliente_explicitam_abas_e_formularios_principais -q`, `git diff --check`, `make web-ci`, `make production-ops-check-strict`, `make uploads-restore-drill`, `make hygiene-check` e `make verify`
- `checkpoint 2026-04-24`: novo slice frontend do Portal Admin Cliente concluído localmente; `web/static/js/cliente/portal_admin_overview_surface.js` deixou de interpolar HTML no resumo executivo, onboarding guiado, pacote comercial, observabilidade e central de pendências, preservando atalhos `abrir-prioridade` e classes visuais por helpers DOM locais; validação local com `node --check web/static/js/cliente/portal_admin_overview_surface.js`, `rg -n "innerHTML|escapeAttr|escapeHtml" web/static/js/cliente/portal_admin_overview_surface.js`, `cd web && PYTHONPATH=. python -m pytest tests/test_smoke.py::test_templates_cliente_explicitam_abas_e_formularios_principais -q`, `git diff --check`, `make web-ci`, `make production-ops-check-strict`, `make uploads-restore-drill`, `make hygiene-check` e `make verify`
- `próximo slice sugerido`: migrar `portal_shell.js` ou as superfícies verticais (`servicos`, `ativos`, `documentos`, `recorrencia`) para DOM seguro, ou alternar para `mesa/service.py`/`chat_index_page.js`; em paralelo, definir o storage externo/LFS definitivo dos assets migrados para executar restore drill real fora do workspace local
...
- `Admin CEO`: extrair onboarding/cadastro e acesso inicial para `web/app/domains/admin/client_onboarding_routes.py`, mantendo o bundle temporário de credenciais coeso.
...
- `make hygiene-check`
- `make verify`
- `PYTHONPATH=. python -m ruff check web/app/domains/admin/client_routes.py web/app/domains/admin/client_catalog_view_routes.py web/app/domains/admin/client_catalog_form_routes.py web/app/domains/admin/client_catalog_import_routes.py web/app/domains/admin/client_catalog_lifecycle_routes.py web/tests/test_admin_client_routes.py`
...
- `make hygiene-check`
- `make verify`
- `PYTHONPATH=. python -m ruff check web/app/domains/admin/client_routes.py web/app/domains/admin/client_onboarding_routes.py web/tests/test_admin_client_routes.py`
- `python -m py_compile web/app/domains/admin/client_routes.py web/app/domains/admin/client_onboarding_routes.py`
- `PYTHONPATH=. python -m pytest web/tests/test_admin_client_routes.py -q -k 'cadastrar_empresa or pacote_inicial or novo_cliente'`
- `PYTHONPATH=. python -m pytest web/tests/test_smoke.py -q -k 'admin or catalogo'`
- `git diff --check`
- `make web-ci`
- `make production-ops-check-strict`
